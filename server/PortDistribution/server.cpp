...
// 申请端口
int PortApply(string uuid)
{
  int port = false;
  PortAll serverPort;
  while(!serverPort.PortValidation(port))
    port = serverPort.PortGenerator();
  serverPort.PortRegister(port, uuid);
  return port;
}
