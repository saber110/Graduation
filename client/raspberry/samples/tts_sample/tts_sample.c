...
{
	int         ret                  = MSP_SUCCESS;
	const char* login_params         = "appid = 59e9f43a, work_dir = .";//登录参数,appid与msc库绑定,请勿随意改动
	/*
	* rdn:           合成音频数字发音方式
	* volume:        合成音频的音量
	* pitch:         合成音频的音调
	...
	*/
	...
	const char* filename             = "tts_sample.mp3"; //合成的语音文件名称
	...
	printf("合成完毕\n");
	char command[100] = {0};
	snprintf(command,sizeof(command),"%s %s","aplay",filename);
	printf("%s\n", command);
	system(command);
